...
Metrics <- R6Class(
  ...
  public = 
    list(
      mse = function() {mean((self$target - self$preds) ^ 2)},
      rmse = function() {sqrt(self$mse())},
  ...
  inherit = Metrics,
   ...
   public =
    list(
      ...
      fit = function() {self$fitted <- lm(x ~ y, data = self$data)},
      predict = function(data) {self$preds <- predict(self$fitted, type = "response", newdata = data)}
    )
)
```

```{r}
aaa <- LinReg$new(diamonds)
```
...
NeuralNet <- R6Class(
  ...
  public =
    list(
      ...
      test = function() {
        preds <- as.double(self$fitted$model$forward(self$test_ds[1:length(test_ds)]$x))
        preds_naif <- rep(mean(as.numeric(torch_cat(list(train_ds$y, valid_ds$y)))), length(self$test_ds))
        
        truth_numeric <- as.double(self$test_ds$y)
        truth_factor <- factor(truth_numeric, levels = c("1", "0"))
        
        mse = rmse_vec(truth = truth_numeric, estimate = preds) ^ 2
        mse_naif <- rmse_vec(truth = truth_numeric, estimate = preds_naif) ^ 2
        
        list(
          auroc = roc_auc_vec(truth = truth_factor, estimate = preds),
          mse = mse,
          mse_skill = 1 - mse / mse_naif
        )
      },
