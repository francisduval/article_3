...
ClassifMetrics <- R6Class(
  ...
  public = 
    list(
      ...
      brier_skill = function() {
        preds_naif <- rep(mean(self$train_targets), length(self$test_targets))
        mse_naif <- mean((self$test_targets - preds_naif) ^ 2)
        1 - self$mse() / mse_naif
      },
...
```{r}
glm_mod <- ClassifGLM$new(train = train, test = bind_rows(valid, test), response = "claim_ind_cov_1_2_3_4_5_6")
glm_mod$fit()
```
  ...
  public = list(
    ...
    initialize = function(train, test, response) {
     ...
     tune = function() {
       ...
       grid <- grid_regular(penalty(), mixture(), levels = c(30, 5))
       recipe <-
